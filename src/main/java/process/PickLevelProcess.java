...
public class PickLevelProcess extends Process {
    ...
    private enum BtnType {
        MODE_CLASSIC, MODE_ULTIMATE, LEVEL, BACK
    }
    ...
    private static class Btn {
        ...
        Btn withLevel(int idx) {
            this.levelIndex = idx;
            return this;
        }

        Btn withImage(Image img) {
            this.image = img;
            return this;
        }
    }
    ...
    private void addBackground() {
        ...
        String filePath = "file:src" + File.separator + "main" + File.separator + "resources"
                + File.separator + "image" + File.separator + "bk12.jpg";
    ...
    private void setupStaticButtons() {
        ...
        Btn classic = new Btn(centerX - 180, topY, cW, cH, Color.color(0, 0, 0, 0.7), "CLASSIC",
                BtnType.MODE_CLASSIC);

        double uW = currentMode == Mode.ULTIMATE ? MODE_W_SEL : MODE_W;
        double uH = currentMode == Mode.ULTIMATE ? MODE_H_SEL : MODE_H;
        Btn ultimate = new Btn(centerX + 180, topY, uW, uH, Color.color(1, 0.85, 0, 0.85),
                "ULTIMATE", BtnType.MODE_ULTIMATE);

        modeButtons.clear();
        modeButtons.add(classic);
        modeButtons.add(ultimate);

        backButton = new Btn(80, height - 60, BACK_W, BACK_H, Color.color(0.8, 0.2, 0.2, 0.85),
                "BACK", BtnType.BACK);
    }
        ...
        if (currentMode == Mode.CLASSIC) {
            ...
            for (int r = 0; r < rows; r++) {
                for (int c = 0; c < cols; c++) {
                    ...
                    Btn b = new Btn(cx, cy, LVL_W, LVL_H, Color.color(1, 1, 1, 0.85),
                            "LEVEL " + (idx + 1), BtnType.LEVEL).withLevel(idx + 1);
                    levelButtons.add(b);
                    idx++;
                }
            }
            ...
            for (int i = 0; i < 2; i++) {
                ...
                Btn b = new Btn(cx, cy, LVL_W, LVL_H, Color.color(1, 1, 1, 0.85),
                        "ULTIMATE " + (i + 1), BtnType.LEVEL).withLevel(i + 14);
                levelButtons.add(b);
            }
        }
    }
    ...
    private void drawButton(Btn b) {
        ...
        Color textColor = (b.image != null) ? Color.WHITE
                : ((b.color.getRed() + b.color.getGreen() + b.color.getBlue()) / 3 > 0.6
                        ? Color.BLACK
                        : Color.WHITE);
        gc.setFill(textColor);
        int fontSize =
                (b.type == BtnType.MODE_CLASSIC || b.type == BtnType.MODE_ULTIMATE) ? 28 : 22;
