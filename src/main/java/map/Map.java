...
public class Map extends GameObject {
    ...
    public void render(GraphicsContext gc) {
        gc.save();
        gc.drawImage(this.getImage(), MAP_ZONE.getX(), MAP_ZONE.y, MAP_ZONE.getWidth(),
                MAP_ZONE.getHeight());
        for (Brick b : bricks) {
    ...
    private void selectMoveBrick() {
        ...
        while (it.hasNext()) {
            ...
            // đưa vào mảng
            arrBrick[(int) Math.round(by / Brick.BRICK_HEIGHT)][(int) Math
                    .round(bx / Brick.BRICK_WIDTH)] = b;

            if (b.movedist != 0) {
                // nếu đã có ô được chọn tự do di chuyển ở hàng này
                // thì đánh số nó
                hasMoveBrick[(int) Math.round(by / Brick.BRICK_HEIGHT)] =
                        (int) Math.round(bx / Brick.BRICK_WIDTH) + 1;
        ...
        for (int i = 0; i < 8; i++) {
            if (hasMoveBrick[i] > 0) {
                ...
                b.left = (int) (MAP_ZONE.getX() + 30
                        + (hasMoveBrick[i] - 1 - k_l) * Brick.BRICK_WIDTH); // biên trái trên màn
                                                                            // hình
                b.right = (int) (MAP_ZONE.getX() + 30
                        + (hasMoveBrick[i] - 1 + k_r) * Brick.BRICK_WIDTH); // biên phải trên màn
                                                                            // hình
            ...
            if (pick != -1 && dist > 1) {
                ...
                b.movedist = 3 * (rand.nextInt(3) - 1); // ta mặc định vận tốc là -3/0/3 khi dịch
                                                        // chuyển trái / phải
                b.left = (int) (Map.MAP_ZONE.getX() + 30 + (pick - l) * (Brick.BRICK_HEIGHT)); // biên
                                                                                               // trái
                                                                                               // trên
                                                                                               // màn
                                                                                               // hình
                b.right = (int) (Map.MAP_ZONE.getX() + 30 + (pick + r) * (Brick.BRICK_WIDTH)); // biên
                                                                                               // phải
                                                                                               // trên
                                                                                               // màn
                                                                                               // hình
    ...
    private void randomRow(PlayingProcess pp) {
        ...
        // Để giảm độ khó mỗi một hàng đẩy xuống sẽ đảm bảo có 1 trong hai trợ giúp lifeUp và
        // pushBrick
        if (help == 0) {
            // thêm máu
    ...
    public enum LevelType {
        CLASSICAL_ONE, CLASSICAL_TWO, CLASSICAL_THREE, CLASSICAL_FOUR, ULTIMATE_ONE, ULTIMATE_TWO
        ...
        // ultimate_one : màn này , các viên gạch có thể tự do bay so với ultimate_one và có thể đẩy
        // gạch
    }
}
